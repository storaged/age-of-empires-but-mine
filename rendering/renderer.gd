...
func _draw_structure(entity: Dictionary) -> void:
	...
	var fill_color: Color = structure_color
	if not constructed:
		fill_color = Color(0.65, 0.49, 0.29, 0.55)
	draw_rect(rect, fill_color, true)
	draw_rect(rect.grow(-12.0), Color("#d4a373"), true)
	draw_rect(rect, Color("#3b2413"), false, 3.0)
	if not constructed:
		var progress: float = 0.0
		var duration: int = maxi(game_state.get_entity_construction_duration_ticks(entity), 1)
		progress = float(game_state.get_entity_construction_progress_ticks(entity)) / float(duration)
...
func _draw_units() -> void:
	...
	for entity_id in entity_ids:
		...
		draw_circle(visual_position, unit_radius, unit_color)
		...
		if game_state.get_entity_carried_amount(entity) > 0:
			draw_circle(
